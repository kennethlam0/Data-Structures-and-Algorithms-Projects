...
public:
  ...
  {
    std::pair<Key_type,Value_type> dummy_value;
    dummy_value.first = k;
    dummy_value.second = Value_type();
    return t.find(dummy_value);
  }
  ...
  {
    //value in map
    if(find(val.first)!=t.end())
    {
      std::pair<Iterator,bool> existing;
      existing.first = find(val.first);
      existing.second = false;
      return existing;
    }
    //value not in map
    else
    {
      t.insert(val);
      std::pair<Iterator, bool> inserted_element;
      inserted_element.first = find(val.first);
...
private:
  ...
  BinarySearchTree<Pair_type,PairComp> t;
};
