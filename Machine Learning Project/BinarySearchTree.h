...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static int size_impl(const Node *node) {
    ...
    int left_size = size_impl(node->left);
    int right_size = size_impl(node->right);
  ...
  static Node *copy_nodes_impl(Node *node) {
    if (node == nullptr){
      return nullptr; //if nullptr then return nullptr
    }
    Node *copy_node = new Node; //create dynamic node to copy into
    copy_node->datum = node->datum; //copy first datum
    copy_node->left = copy_nodes_impl(node->left); //copy the left
    copy_node->right = copy_nodes_impl(node->right); //copy the right

    return copy_node; //return pointer to the root
  }
  ...
  static void destroy_nodes_impl(Node *node) {
    ...
    //need to figure out how to delete the nodes from the bottom up
    destroy_nodes_impl(node->left);
    destroy_nodes_impl(node->right);
    delete node; //i think this should work...
    //recursively moves down the tree and then deletes nodes at bottom first 
  }
  ...
  static Node * min_greater_than_impl(Node *node, const T &val, Compare less) {
    ...
    { // if val is less than the current node's datum
      Node* left_piece = min_greater_than_impl(node->left, val, less);
      if(left_piece != nullptr)
      {
        return left_piece;
      }
      else 
