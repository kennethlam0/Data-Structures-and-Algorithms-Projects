...
void compute_energy_matrix(const Image* img, Matrix* energy) 
{
  ...
  for(int i = 1; i < Image_height(img) - 1; ++i)
  {
    for(int j = 1; j < Image_width(img) - 1; ++j)
    {
      ...
      *Matrix_at(energy, i, j) = energy_value; 
...
void compute_vertical_cost_matrix(const Matrix* energy, Matrix *cost) 
{
  ...
  for(int i = 1; i < Matrix_height(cost); ++i)
  {
    for(int j = 0; j < Matrix_width(cost); ++j)
    {
      if(j == 0)
      {
        int cost_a = *Matrix_at(energy, i, j) 
        + Matrix_min_value_in_row(cost, i - 1, j, j + 2);
        *Matrix_at(cost, i, j) = cost_a;
      }
      else if(j == Matrix_width(cost))
      {
        int cost_b = *Matrix_at(energy, i, j) 
        + Matrix_min_value_in_row(cost, i - 1, j - 1, j + 1);
        *Matrix_at(cost, i, j) = cost_b;
      }
      else
      {
        int cost_c = *Matrix_at(energy, i, j) 
        + Matrix_min_value_in_row(cost, i - 1, j-1, j + 2);
        *Matrix_at(cost, i, j) = cost_c;
      }
    }
  }
}
...
void find_minimal_vertical_seam(const Matrix* cost, int seam[]) {
  ...
  int curr_column = start_column;
  seam[Matrix_height(cost) - 1] = start_column;
  for(int i = Matrix_height(cost)-2; i>=0; --i)
  {
    curr_column = 
    Matrix_column_of_min_value_in_row(cost, i, curr_column - 1, curr_column + 2);
    seam[i] = curr_column;
  }
}
